...
#sudo ip netns exec host0 ip addr add 128.10.126.50/24 dev eno1
#sudo ip netns exec host0 ip route add default via 128.10.126.1 dev eno1
#sudo ip netns exec host0 ip -f inet addr show
#sudo ip netns exec host0 /usr/sbin/sshd -o PidFile=/run/sshd-host0.pid -o ListenAddress=128.10.126.50
...
sudo ip netns exec host1 ip addr add 128.10.126.51/24 dev eno2
sudo ip netns exec host1 ip route add default via 128.10.126.1 dev eno2
sudo ip netns exec host1 ip -f inet addr show
sudo ip netns exec host1 /usr/sbin/sshd -o PidFile=/run/sshd-host1.pid -o ListenAddress=128.10.126.51
...
sudo ip netns exec host2 ip addr add 128.10.126.52/24 dev enp1s0f0
sudo ip netns exec host2 ip route add default via 128.10.126.1 dev enp1s0f0
sudo ip netns exec host2 ip -f inet addr show
sudo ip netns exec host2 /usr/sbin/sshd -o PidFile=/run/sshd-host2.pid -o ListenAddress=128.10.126.52
...
sudo ip netns exec host3 ip addr add 128.10.126.53/24 dev enp1s0f1
sudo ip netns exec host3 ip route add default via 128.10.126.1 dev enp1s0f1
sudo ip netns exec host3 ip -f inet addr show
sudo ip netns exec host3 /usr/sbin/sshd -o PidFile=/run/sshd-host3.pid -o ListenAddress=128.10.126.53
...
sudo ip netns exec host4 ip addr add 128.10.126.54/24 dev enp132s0f0
...
sudo ip netns exec host4 /usr/sbin/sshd -o PidFile=/run/sshd-host4.pid -o ListenAddress=128.10.126.54
...
sudo ip netns exec host5 ip addr add 128.10.126.55/24 dev enp132s0f1
...
sudo ip netns exec host5 /usr/sbin/sshd -o PidFile=/run/sshd-host5.pid -o ListenAddress=128.10.126.55
...
sudo ip netns exec host6 ip addr add 128.10.126.56/24 dev enp132s0f2
...
sudo ip netns exec host6 /usr/sbin/sshd -o PidFile=/run/sshd-host6.pid -o ListenAddress=128.10.126.56
...
#sudo ip netns exec host7 ip addr add 128.10.126.57/24 dev enp132s0f3
...
#sudo ip netns exec host7 /usr/sbin/sshd -o PidFile=/run/sshd-host7.pid -o ListenAddress=128.10.126.57
