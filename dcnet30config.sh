...
#sudo ip netns exec host0 ip addr add 128.10.126.74/24 dev eno1
#sudo ip netns exec host0 ip route add default via 128.10.126.1 dev eno1
#sudo ip netns exec host0 ip -f inet addr show
#sudo ip netns exec host0 /usr/sbin/sshd -o PidFile=/run/sshd-host0.pid -o ListenAddress=128.10.126.74
...
sudo ip netns exec host1 ip addr add 128.10.126.75/24 dev eno2
sudo ip netns exec host1 ip route add default via 128.10.126.1 dev eno2
sudo ip netns exec host1 ip -f inet addr show
sudo ip netns exec host1 /usr/sbin/sshd -o PidFile=/run/sshd-host1.pid -o ListenAddress=128.10.126.75
...
sudo ip netns exec host2 ip addr add 128.10.126.76/24 dev enp1s0f0
sudo ip netns exec host2 ip route add default via 128.10.126.1 dev enp1s0f0
sudo ip netns exec host2 ip -f inet addr show
sudo ip netns exec host2 /usr/sbin/sshd -o PidFile=/run/sshd-host2.pid -o ListenAddress=128.10.126.76
...
sudo ip netns exec host3 ip addr add 128.10.126.77/24 dev enp1s0f1
sudo ip netns exec host3 ip route add default via 128.10.126.1 dev enp1s0f1
sudo ip netns exec host3 ip -f inet addr show
sudo ip netns exec host3 /usr/sbin/sshd -o PidFile=/run/sshd-host3.pid -o ListenAddress=128.10.126.77
...
sudo ip netns exec host4 ip addr add 128.10.126.78/24 dev enp132s0f0
...
sudo ip netns exec host4 /usr/sbin/sshd -o PidFile=/run/sshd-host4.pid -o ListenAddress=128.10.126.78
...
sudo ip netns exec host5 ip addr add 128.10.126.79/24 dev enp132s0f1
...
sudo ip netns exec host5 /usr/sbin/sshd -o PidFile=/run/sshd-host5.pid -o ListenAddress=128.10.126.79
...
sudo ip netns exec host6 ip addr add 128.10.126.80/24 dev enp132s0f2
...
sudo ip netns exec host6 /usr/sbin/sshd -o PidFile=/run/sshd-host6.pid -o ListenAddress=128.10.126.80
...
sudo ip netns exec host7 ip addr add 128.10.126.81/24 dev enp132s0f3
...
sudo ip netns exec host7 /usr/sbin/sshd -o PidFile=/run/sshd-host7.pid -o ListenAddress=128.10.126.81
